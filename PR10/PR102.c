...
static int sleep_periodic_absolute(struct timespec *deadline, long period_ms)
{
    ...
    {
        /*
         * Після сигналу використовуємо той самий абсолютний deadline.
         * Це не накопичує похибку часу.
         */
    }
...
int main(void)
{
    ...
    printf("PID=%ld. В іншому терміналі виконайте: kill -USR1 %ld\n",
           (long)getpid(), (long)getpid());

    puts("Relative sleep на 5 секунд через nanosleep restart loop...");
    ...
    printf("Relative sleep завершено. got_usr1=%d\n", got_usr1);

    puts("Тепер 5 періодичних tick через absolute clock_nanosleep deadlines...");
